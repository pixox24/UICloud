...
import { Filter, List, Loader2, Search, Star, X } from "lucide-react";
...
const CARD_META_VISIBILITY_STORAGE_KEY = "ui-library:browse-show-card-meta";
...
function BrowsePageContent() {
  ...
  const [cardMetaPreferenceReady, setCardMetaPreferenceReady] = useState(false);
  ...
  useEffect(() => {
    try {
      const storedValue = window.localStorage.getItem(CARD_META_VISIBILITY_STORAGE_KEY);
      setShowCardMeta(storedValue === "1");
    } finally {
      setCardMetaPreferenceReady(true);
    }
  }, []);

  useEffect(() => {
    if (!cardMetaPreferenceReady) {
      return;
    }

    window.localStorage.setItem(
      CARD_META_VISIBILITY_STORAGE_KEY,
      showCardMeta ? "1" : "0"
    );
  }, [cardMetaPreferenceReady, showCardMeta]);

  const fetchFavorites = useCallback(() => {
    fetch("/api/favorites?favorites=1")
      .then((response) => response.json())
      .then((data) => {
        const ids = new Set<number>((data.assets || []).map((asset: Asset) => asset.id));
        setFavoriteIds(ids);
      })
      .catch(() => undefined);
  }, []);
  ...
  const toggleFavorite = async (assetId: number) => {
    ...
    try {
      if (isFavorite) {
        ...
        setFavoriteIds((current) => new Set(current).add(assetId));
      }
    } catch {
      return;
    }
  ...
  const filterButtonLabel = showFiltersPanel ? "收起筛选" : "显示筛选";
  const metaButtonLabel = showCardMeta ? "隐藏卡片信息" : "显示卡片信息";
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto w-full max-w-[1680px] space-y-5 px-3 py-4 sm:px-4 lg:px-5">
        <div className="flex items-center justify-end gap-2">
          <button
            ...
            className={`inline-flex h-11 w-11 items-center justify-center rounded-full border transition-colors ${
              showFiltersPanel || hasActiveFieldFilters
          ...
          <button
            type="button"
            onClick={() => setShowCardMeta((current) => !current)}
            aria-label={metaButtonLabel}
            aria-pressed={showCardMeta}
            title={metaButtonLabel}
            className={`inline-flex h-11 w-11 items-center justify-center rounded-full border transition-colors ${
              showCardMeta
                ? "border-primary/35 bg-primary/10 text-primary"
                : "border-border/80 bg-card/60 text-muted-foreground hover:border-primary/30 hover:text-foreground"
        ...
        {showFiltersPanel ? (
          <div className="rounded-[28px] border border-border/70 bg-card/45 p-4 shadow-[0_24px_60px_-40px_rgba(0,0,0,1)] backdrop-blur-xl">
